...
@torch.no_grad()
def advect_diffuse_spectral(u0: torch.Tensor, v: torch.Tensor, kappa: float, dt: float, steps: int) -> List[torch.Tensor]:
    H, W = u0.shape
    device = u0.device
    ky, kx = _fft_freqs(H,W,device)
    K2 = ky**2 + kx**2
    phase = torch.exp(-1j*(v[0]*kx + v[1]*ky)*dt)   # 平流相位
    decay = torch.exp(-kappa*K2*dt)                 # 扩散衰减
